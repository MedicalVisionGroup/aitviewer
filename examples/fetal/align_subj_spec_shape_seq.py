...
def main(exp_dir, subj_name_list, step_idx, use_zero_pose_and_shape_mag):
    # create viewer
    ...
    for i, name in enumerate(subj_name_list):
        ...
        if use_zero_pose_and_shape_mag:
            # use zero pose for visualize the body shape
            # magnify beta by scale = 1.2 for better visualization
            body_pose_seq = np.zeros_like(body_pose_seq)
            global_orient_seq = np.zeros_like(global_orient_seq)
            transl_seq = np.zeros_like(transl_seq)
            subj_spec_shape = subj_spec_shape * 1.2
            # subj_spec_shape = np.zeros_like(subj_spec_shape)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_zero_pose_and_shape_mag", type=str, default="False")
    ...
    main(args.exp_dir, subj_name_list, args.step_idx, args.use_zero_pose_and_shape_mag)
