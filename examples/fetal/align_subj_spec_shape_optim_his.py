...
def main(data_dir, exp_dir, name, frame_idx_list, step_idx, model_path=None):
    # read data
    ...
    global_orient_seq_hist = np.load(osp.join(pose_dir, "global_orient_seq_his.npy"))
    ...
    if model_path is None:
        ...
        # use provided model and beta history
        last_beta_hist_path = osp.join(
            subj_dir, last_unposed_folder_name, "beta_his.npy"
        )
        subj_spec_beta = np.load(last_beta_hist_path)[-1]
        betas = torch.from_numpy(subj_spec_beta).unsqueeze(0).float()

        smpl_layer = SMPLLayer(
            model_type="smpl",
            gender="infant",
            num_betas=betas.shape[1],
            fetal_smpl_data_dict_path=model_path,
        )
    ...
    for i, f_idx in enumerate(frame_idx_list):
        ...
        mesh_seq = Meshes(verts, faces, color=(0.5, 0.5, 0.5, 1), name="BodySegm")
