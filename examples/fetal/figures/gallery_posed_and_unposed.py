...
def main(exp_dir, subj_name_list, step_idx, smil_data_path):
    # the mean pose + zero translation/global orientation was ugly
    prior_mean = np.load(osp.join(smil_data_path, "smil_pose_prior_converted_mean.npy"))

    # use the first pose from map-c507 (init)
    c507_exp_subj_pose_dir = osp.join(exp_dir, "subj_spec", "MAP-C507", f"init_posed")
    c507_tr = np.load(osp.join(c507_exp_subj_pose_dir, "transl_seq_his.npy"))[-1][0]
    c507_go = np.load(osp.join(c507_exp_subj_pose_dir, "global_orient_seq_his.npy"))[
        -1
    ][0]
    c507_bp = np.load(osp.join(c507_exp_subj_pose_dir, "body_pose_seq_his.npy"))[-1][0]

    # create viewer
    Viewer.window_type = "pyqt6"
    v = Viewer()

    num_col = int(np.sqrt(len(subj_name_list)))
    num_col = math.ceil(num_col)
    ...
    for i, name in enumerate(subj_name_list):
        ...
        # add a flattened baby by the size
        # this_smpl_seq_flat = SMPLSequence(
...
if __name__ == "__main__":
    ...
    smil_data_path = (
        "/Users/yingchengliu/Dropbox (MIT)/code/MIT & work/"
        "fetal_pose/skel-mac/smplx/models"
    )
    main(args.exp_dir, subj_name_list, args.step_idx, smil_data_path, args.flatten)
